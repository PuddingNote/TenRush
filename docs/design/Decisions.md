...
## 2026-08-15 타일 입체감 (그림자 + 셰이딩 오버레이)

- 타일이 너무 밋밋하다는 피드백으로, 평면 단색 사각형 → **레이어 3장**으로
  교체(`TileButton.cs`): Shadow(뒤, 타일보다 1.25배 크고 6px 아래) → Base(실제
  탭 판정 면, 흰색 스프라이트를 HSL 색으로 틴트) → ShadingOverlay(틴트 없이
  그대로 덮음 — 그 자체가 반투명 위쪽 밝음/아래쪽 어두움 그라디언트라서 색이
  뭐든 자연스럽게 입체감을 줌).
- 타일 베이스 스프라이트를 `white_square_128`(각짐) → `white_square_rounded_128`
  (둥근 모서리)로 교체.
- `UiSpriteLibrary`에 이미 만들어져 있던 에셋(`Assets/Resources/UI/UiSpriteLibrary.asset`)
  이라서, 이번엔 사용자에게 인스펙터 작업을 부탁하는 대신 **Claude Code가
  직접 .asset YAML을 수정**해서 스프라이트 참조를 걸었다(각 PNG의 .meta에서
  guid/fileID를 미리 읽어서 정확한 값으로 채움) — AudioLibrary/UiSpriteLibrary를
  처음 만들 때와 달리, 에셋이 이미 존재하고 구조를 안다면 이 방식이 더 빠름.
- 그림자/오버레이 스프라이트가 비어 있으면(향후 슬롯을 지우는 경우) 그 레이어
  자체를 안 만들도록 방어 처리 — 불투명한 흰 사각형으로 덮여버리는 사고 방지.
